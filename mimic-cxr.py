...
flags.DEFINE_float("lr", 0.25, "Learning rate.")
flags.DEFINE_float("weight_decay", 1e-3, "L2 weight decay.")
flags.DEFINE_float("momentum", 0.99, "Momentum parameter.")
...
def get_compiled_model(
    train_steps: int, imagenet_weights: bool = False, num_classes: int = 14
):
    ...
    model.compile(
        ...
        metrics=[
            ...
            ),  # CheXpert evaluation protocol: macro AUROC averaged over the 5 CheXpert labels
            keras.metrics.AUC(
...
def main(argv):
    ...
    if FLAGS.full_dataset:
        ...
        STEPS = (
            int(len(train_dataset) * FLAGS.subset_ratio)
            // FLAGS.batch_size
            * FLAGS.epochs
        )
        model = get_compiled_model(train_steps=STEPS, imagenet_weights=imagenet_weights)
